/******************************************************************************
  ...
  在Z-Stack中使用MAC TIMER作为osal时钟源，即可实现定时也可以实现低功耗，
  现在将MAC Timer更换为Sleep Timer，两者功能等同，精度相近。
...
uint32 macMcuPrecisionCount(void);
  ...
  // 1ms equals 32 ticks
  #define CONVERT_ST_TICKS_TO_MS_ELAPSED_REMAINDER( x, y, z ) st( \
    y += x >> 5;       /**/                                \
    z = x & 0x1F;                                                \
     )
...
void osalTimeUpdate( void )
{
  ...
  // Get the free-running count of sleep timer ticks
  halSleepGetCounter(&tmp);
  //HAL_EXIT_CRITICAL_SECTION(intState);
  
  // 在0xFFFFFF处溢出暂时没有处理
  //uint32 elapsedTicks = (tmp - previousMacTimerTick);
  
  if ( tmp != previousMacTimerTick )
  {
    // Calculate the elapsed ticks of the free-running timer.
    //ticks320us = (tmp - previousMacTimerTick) & 0xffffffffu;
    elapsedTicks = (tmp - previousMacTimerTick)& 0xffffffffu;;
    ...
    if ( elapsedTicks > 31 )
    {
      ...
      tmp = (elapsedTicks * 1) + remUsTicks;
      
      // Convert the 320 us ticks into milliseconds and a remainder
      CONVERT_ST_TICKS_TO_MS_ELAPSED_REMAINDER( tmp, elapsedMSec, remUsTicks );
