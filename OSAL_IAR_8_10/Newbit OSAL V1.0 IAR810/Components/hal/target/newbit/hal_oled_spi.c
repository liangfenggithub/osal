...
/*  Define for Fonts & Pictures*/   
#define    FONT_FLASH_PAGE  8
#define    LOGO_FLASH_PAGE  FONT_FLASH_PAGE 
#define    LOGO_FLASH_OFFSET_BASE  0x900


/* SOFT or HARD SPI selected */
#define SOFT_SPI  0
#define HARD_SPI  1

#define SPI_USED  HARD_SPI    // Select which spi interface you want to use
...
void HalLcd_HW_Write_pic(uint8 page, uint16 offset);
...
/**************************************************************************************************
 * @fn      HalLcdDisplayPicture
 *
 * @brief   Display logo
 *
 * @param   the logo number  you want to display , 0 - txt, 1 - pic, other - txt
 *
 * @return  void
 **************************************************************************************************/
void HalLcdDisplayPicture(uint8 n)
{
#if (HAL_LCD == TRUE)
  uint8    page = LOGO_FLASH_PAGE;
  uint16   offset = LOGO_FLASH_OFFSET_BASE;
  ...
  HalLcd_HW_Write_pic(page, offset);
...
static void halLcd_ConfigSPI(void)
{
...
#if (SPI_USED == SORT_SPI)
  // 3-WIRE SPI , 每次都需要传输9bit, 因此使用模块SPI更加方便，但速度较慢
   ...
   */
  // 15 1MHZ, 16 2MHZ, 17 4MHZ, ...
  //baud_exponent = 15; // 1Mhz
  //baud_mantissa =  0; // 1Mhz
  
  //baud_exponent = 17; // 4Mhz
  //baud_mantissa =  0; // 4Mhz
    
  //baud_exponent = 19; // 16Mhz
  //baud_mantissa =  0; // 16Mhz
  
  baud_exponent = 18; // 8Mhz
  baud_mantissa =  0; // 8Mhz  
...
uint8 i = 8;
  ...
  spiByte[1] |= (byte&0x01) ? 0x80 : 0x00;
  
  LCD_SPI_TX(spiByte[0]);
  LCD_SPI_WAIT_RXRDY();
  
  LCD_SPI_TX(spiByte[1]);
  LCD_SPI_WAIT_RXRDY();
...
/**************************************************************************************************
 * @fn      HalLcd_HW_Write_pic
 *
 * @brief   write a picture array to oled
 *
 * @param   uint8 page, uint16 offset
 *
 * @return  void
 **************************************************************************************************/
void HalLcd_HW_Write_pic(uint8 page, uint16 offset)
{
  ...
  {    
    HalLcd_HW_SetCursor(column, 0);  
    
    HalFlashRead(FONT_FLASH_PAGE, oft, picdot, 128);
    p = picdot;
    for(int i = 0; i < 128; ++i)
    {
      HalLcd_HW_Write( *p++ );
