...
/* convert mesc to sleep timer ticks */
#define HAL_SLEEP_MS_TO_ST_TICK(ms)         (((uint32)(ms)) << 5)
...
static uint32 maxSleepLoopTime =  HAL_SLEEP_MS_TO_ST_TICK(MAX_SLEEP_TIME);
...
{
  if( rolloverTime > HAL_SLEEP_MS_TO_ST_TICK(MAX_SLEEP_TIME) )
  {
    maxSleepLoopTime = HAL_SLEEP_MS_TO_ST_TICK(MAX_SLEEP_TIME);
  }
...
void halSleep( uint16 osal_timeout )
{
  ...
  /* get next OSAL timer expiration converted to sleep timer ticks */
  timeout = HAL_SLEEP_MS_TO_ST_TICK(osal_timeout);

  // mac in no longer exsit
   ...
   */
#if ZG_BUILD_ENDDEVICE_TYPE && defined (NWK_AUTO_POLL)
  if ((timeout > HAL_SLEEP_MS_TO_ST_TICK(PM_MIN_SLEEP_TIME)) ||
...
#endif
  {
    ...
    //if (halSleepPconValue != 0 && MAC_PwrOffReq(MAC_PWR_SLEEP_DEEP) == MAC_SUCCESS) //del mark
    if (halSleepPconValue != 0)
    {
      ...
      if (timeout != 0)
      {
        if (timeout > HAL_SLEEP_MS_TO_ST_TICK( MAX_SLEEP_TIME ))
        {
          timeout -= HAL_SLEEP_MS_TO_ST_TICK( MAX_SLEEP_TIME );
          halSleepSetTimer(HAL_SLEEP_MS_TO_ST_TICK( MAX_SLEEP_TIME ));
        }
        else
        {
          /* set sleep timer */
       ...
       */
      HAL_SLEEP_SET_POWER_MODE();   // ENTER low power mode
...
/**************************************************************************************************
 * @fn          halSleepGetCounter
 *
 * @brief       Read sleep timer counter
 *              
 *
 * input parameters
 *
 * @param       ticks - potiner to the value of sleep timer counter
 *
 * output parameters
 *
 * None.
 ...
 **************************************************************************************************/
void halSleepGetCounter(uint32* ticks)
{
  ((uint8 *) ticks)[UINT32_NDX0] = ST0;
  ((uint8 *) ticks)[UINT32_NDX1] = ST1;
  ((uint8 *) ticks)[UINT32_NDX2] = ST2;
